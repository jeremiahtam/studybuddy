...
if(isset($_POST['send-msg'])){
  ...
  if($msg_uname==$user){
		...
		if($num_rows_check_conv==0){//if no conversation exist, add a new conversation
			...
			$deleted_conv_query = mysqli_query($conn,"SELECT * FROM deleted_conversations WHERE conversation_id='$conv_id'");
			...
			if($deleted_conv_rows==1){
				$deleted_conv_details = mysqli_fetch_array($deleted_conv_query);
				$deleted_conv_id = $deleted_conv_details['id'];
				$remove_deleted_conv = mysqli_query($conn,"DELETE FROM deleted_conversations WHERE id='$deleted_conv_id'");
			  }
		  }
		$typed_msg = $_POST['typed_msg'];
		$insert_message = mysqli_query($conn,"INSERT INTO msg_conversations VALUES('','$conv_id','$typed_msg','text','$user','$msg_uname','no','$date','$time')");
		//update the date and time of the conversation
		$update_conv_time = mysqli_query($conn,"UPDATE conversations SET date='$date',time='$time' WHERE id='$conv_id'");
